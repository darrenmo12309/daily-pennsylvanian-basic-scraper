...
I changed the scraper so it would get the top sports article in the sports section of the homepage. In doing this, I had to make my navigation logic very precise, as there were more complexities in the path my scraper had to follow to get to the top sports article. First I had to make use of chained find() and find_all() functions, in order to go down the tree of HTML elements. In order to go the specific path of the tree I wanted, I made sure to specify which child element I wanted specifically. In the case that there was more than one child element, I use the find_all function and chose the one in the returned list I wanted. Finally, in the case I wanted to only get elements in the very level of the tree below the current element I was on, I had to use the recursive = false parameter so that I could actually know where the element I wanted was in my list (otherwise there would've been too many div elements with class = "col-sm-6"). Then when I finally got to the top sports article, I simply did the same as before, I extracted the text.

## Cron Syntax Explanation

The cron syntax allows 5 slots for controlling when the workflow runs. The first slot corresponds to the minute, the second to the hour, the third to the day of the month, the fourth to the month, and the fifth to the day of the week. Special values also exist, * corresponds to any time for the slot its located in, "," separates multiple values in one slot, and "-" can be used to give a range of values.
